...
const char *connectionString = "connstring";

const char *wifi_ssid = "wifiname";
const char *wifi_pwd = "wifipass";

const char *deviceLocation = "Helsinki";
...
const char *certificateFingerprint = "asdf";
...
DigitalPin powerPin(D8);
//DigitalPin2 powerPin2(D7);

// uncomment required sensor

//Sensor sensor;  // Fake sample environmental data

//WLEVEL sensor(&powerPin);
CHIRP sensor(&powerPin);

//HTU21DF sensor(&powerPin);
...
IPAddress timeServer(194, 100, 100, 152);

void initDeviceConfig()
{                                   // Example device configuration
  device.deepSleepSeconds = 3600;   // 900;  // 1h 3600;         // if greater than zero with call ESP8266 deep sleep (default is 0 disabled). GPIO16 needs to be tied to RST to wake from deepSleep. Causes a reset, execution restarts from beginning of sketch
  device.publishRateInSeconds = 90; // limits publishing rate to specified seconds (default is 90 seconds).  Connectivity problems may result if number too small eg 2
